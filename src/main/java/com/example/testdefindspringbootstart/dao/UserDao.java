...
public interface UserDao extends JpaRepository<User, Integer> {
    ...
    /**
     jpa 自定义查询方法一定要按照规范来定义方法名
     */
    @Cacheable
    User findUserByUserName(String userName);

    User findUserByUserNameAndAge(String userName,Integer age);
...
@Cacheable：配置了findByName函数的返回值将被加入缓存。同时在查询时，会先从缓存中获取，
若不存在才再发起对数据库的访问。该注解主要有下面几个参数：
...
它每次都会真是调用函数，所以主要用于数据新增和修改操作上。它的参数与@Cacheable类似，具体功能可参考上面对@Cacheable参数的解析
